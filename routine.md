...
## Telegram Intake
- เมื่อ CEO ส่งบันทึก ความคิด หรือสิ่งที่ต้องทำผ่าน Telegram ให้เลขาแยกประเภทข้อมูลก่อนบันทึก
...
- If the message is a concrete CEO assignment, create the Paperclip task first, then reply in Telegram with Task title, Task ID, Assignee, Priority, and Status.
...
- If required credentials, approved chat target, Paperclip task creation, Telegram delivery, or GitHub push fails, escalate the exact failure to CEO and record the blocker in the issue.

## Telegram Resource Control Update - 2026-07-26
- CEO clarified that the secretary does not need to wait for Telegram replies continuously if doing so wastes tokens or runtime resources.
...
- Do not run an always-on high-frequency polling loop unless CEO explicitly approves the resource cost.
...
- CEO approved a resource-saving cadence: check and process Telegram intake every 4 hours.
- Every morning, send CEO a concise reminder of tasks that need follow-up.
- Morning follow-up reminders should include open tasks, pending CEO confirmations, and business items marked Follow-up or Pending in the Company Brain.
- Do not send idle acknowledgements between scheduled checks unless there is a new processed update, blocker, or explicit CEO request.
...
- `company/follow_up.md` is the canonical follow-up list for CEO and ChatGPT queries.
...
- Direct CEO Calendar event creation is not active yet.
...
- INF-224 is the unblock path: Paperclip admin/engineering must provide Calendar-capable access and CEO must authorize the correct calendar before the secretary can create events directly.
